...
from Python_Web.hw40.src.database.db import get_db
from Python_Web.hw40.src.schemas import ReviewCreate, ReviewResponse
from Python_Web.hw40.src.repository import reviews as repository_reviews
from Python_Web.hw40.src.auth import get_current_user
...
async def create_review(
    review: ReviewCreate,
    db: AsyncSession = Depends(get_db),
    current_user: tuple = Depends(get_current_user),  # reviewer_id береться з токена
):
    """Створити відгук після завершеного обміну."""
    reviewer_id, _ = current_user
