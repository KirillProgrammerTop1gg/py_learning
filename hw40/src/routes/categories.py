...
async def read_categories(
    ...
    only_active: bool = Query(
        True, description="Якщо true — повертає лише активні категорії"
    ),
    ...
    """
    categories = await repository_categories.get_categories(
        skip, limit, only_active, db
    )
...
async def read_category(
    category_id: int = Path(
        ..., ge=1, description="Унікальний ідентифікатор категорії"
    ),
    db: AsyncSession = Depends(get_db),
):
...
async def read_category_by_slug(
    slug: str = Path(
        ..., description="URL-slug категорії (напр. 'programming', 'music')"
    ),
    db: AsyncSession = Depends(get_db),
):
...
async def read_category_skills(
    category_id: int = Path(
        ...,
        ge=1,
        description="Унікальний ідентифікатор категорії, чиї навички потрібно отримати",
    ),
    db: AsyncSession = Depends(get_db),
):
...
async def update_category(
    category_id: int = Path(
        ...,
        ge=1,
        description="Унікальний ідентифікатор категорії, яку потрібно оновити",
    ),
    ...
    if category_update.name:
        existing = await repository_categories.get_category_by_name(
            category_update.name, db
        )
    ...
    if category_update.slug:
        existing = await repository_categories.get_category_by_slug(
            category_update.slug, db
        )
        if existing and existing.id != category_id:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    db_category = await repository_categories.update_category(
        category_id, category_update, db
    )
    if not db_category:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def delete_category(
    category_id: int = Path(
        ...,
        ge=1,
        description="Унікальний ідентифікатор категорії, яку потрібно видалити",
    ),
    db: AsyncSession = Depends(get_db),
):
    """
